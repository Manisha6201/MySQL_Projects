...
SELECT Region, sum(Quantity * UnitPrice) as "total sales" from Sales
GROUP BY Region
...
SELECT ProductID, sum(quantity * UnitPrice) as "total sales amount of more than 1000" from Sales
GROUP BY ProductID
...
SELECT EXTRACT(MONTH FROM SaleDate) as month, sum(Quantity * UnitPrice) as "Total Sales" from Sales
GROUP BY EXTRACT(MONTH FROM SaleDate);

/*Q12. Find the regions where the average unit price is more than 200.*/
SELECT Region, avg(UnitPrice) as "average unit price" from Sales
GROUP BY Region
